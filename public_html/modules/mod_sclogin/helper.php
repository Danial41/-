...
class ModScloginHelper
{
    ...
    public function setupTheme()
    {
        ...
        if($this->params->get('loadFontAwesome') && $this->params->get('loadFontAwesome'))
            $this->doc->addStyleSheet(JURI::base(true) . '/media/sourcecoast/css/fontawesome/css/font-awesome.min.css');
...
// common for J!, JomSocial, and Virtuemart
    ...
    private function getForgotButton($showForgotButtonType, $forgotSharedLink, $forgotButtonLink, $forgotSharedText, $forgotButtonText, $registerType)
    {
        ...
        if($showForgotButtonType == 'button_black' || $showForgotButtonType == 'button_white')
        {
            ...
            {
                $forgotButton = '<a href="' . $forgotSharedLink . '" class="forgot btn width-auto hasTooltip" tabindex="-1" data-placement="right" title="' . $forgotSharedText . '" aria-label="'.$forgotSharedText.'"><i class="'. $question . $buttonImageColor . '"></i></a>';
            }
            else
            {
...
function showpsw() {
  ...
  if (x.type === "password") {
    x.type = "text";
    eye.className = "{$eyeClose} field-icon";
  } else {
    x.type = "password";
    eye.className = "{$eye} field-icon";
  }
}</script>
EOT;
            $showButton .= '<a onclick="showpsw()"><span class="'.$eye.' field-icon" id="sc-eye-'.$passwordId.'"></span></a>';
        }
        return $showButton;
    }
